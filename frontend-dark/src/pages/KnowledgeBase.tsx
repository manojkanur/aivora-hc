...
export default function KnowledgeBase() {
  ...
  return (
    <div className="p-5 sm:p-8 max-w-7xl mx-auto space-y-7">
      ...
      <div className="rounded-2xl border border-[#2A3648] bg-[#0f1117] p-4 sm:p-5 space-y-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#0B1220] border border-[#2A3648] rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-slate-500 focus:border-blue-500/50 focus:outline-none"
          />
        </div>
...
function ArticleCard({ article, onClick, isAdmin, onEdit }: { article: Article; onClick: () => void; isAdmin: boolean; onEdit: () => void }) {
  ...
  return (
    <motion.div
      whileHover={{ y: -2 }}
      className="relative rounded-2xl border border-[#2A3648] bg-[#0f1117] p-4 sm:p-5 hover:border-[#2a3048] hover:bg-[#111420] transition-all group"
    >
      ...
      {isAdmin && (
        <button
          ...
          className="absolute top-3 right-3 inline-flex items-center gap-1 rounded-lg border border-[#2A3648] bg-[#0B1220]/80 px-2 py-1 text-[10px] font-semibold text-slate-300 hover:text-white hover:border-blue-500/40 opacity-0 group-hover:opacity-100 transition-opacity"
          title="Edit article"
        >
...
function ArticleDetail({
  ...
  return (
    <div className="p-5 sm:p-8 max-w-5xl mx-auto space-y-7">
      <div className="flex items-center gap-3">
        ...
        {isAdmin && (
          <div className="ml-auto flex items-center gap-2">
            <button
              onClick={onEdit}
              className="inline-flex items-center gap-2 rounded-lg border border-[#2A3648] bg-[#0f1117] px-3 py-1.5 text-xs font-semibold text-slate-200 hover:text-white hover:border-blue-500/40"
            >
      ...
      <article className="rounded-2xl border border-[#2A3648] bg-[#0f1117] p-6 sm:p-8">
        ...
        <div className="flex flex-wrap gap-2 mt-8 pt-6 border-t border-[#2A3648]">
          {article.tags.map(t => (
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {relatedStudios.map(s => (
              <Link key={s.id} to="/skills" className="rounded-2xl border border-[#2A3648] bg-[#0f1117] p-4 hover:border-[#2a3048] hover:bg-[#111420] transition-all">
      ...
      {related.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {related.map(r => (
              <button key={r.id} onClick={() => { onBack(); setTimeout(() => window.scrollTo(0, 0), 0) }} className="text-left rounded-2xl border border-[#2A3648] bg-[#0f1117] p-4 hover:border-[#2a3048] hover:bg-[#111420] transition-all">
...
function ArticleEditor({ article, onClose, onSaved }: {
  ...
  return (
    <div className="p-5 sm:p-8 max-w-4xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl border border-[#2A3648] bg-[#0f1117] p-5 sm:p-6 space-y-5">
        <Field label="Title">
          <input
            value={draft.title}
            onChange={e => patch({ title: e.target.value })}
            className="w-full bg-[#0B1220] border border-[#2A3648] rounded-xl px-3 py-2.5 text-sm text-white focus:border-blue-500/50 focus:outline-none"
            ...
            {FRAMEWORK.dimensions.map(d => {
              ...
              return (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg border text-[11px] font-medium transition-colors ${on ? 'bg-blue-500/15 border-blue-500/40 text-blue-200' : 'bg-[#0B1220] border-[#2A3648] text-slate-400 hover:text-white'}`}
                >
                  {d.name}
                </button>
              )
      ...
      <div className="rounded-2xl border border-[#2A3648] bg-[#0f1117] p-5 sm:p-6 space-y-4">
        <div className="flex items-center gap-3">
          <h2 className="text-sm font-semibold text-white">Sections</h2>
          <button
            onClick={addSection}
            className="ml-auto inline-flex items-center gap-1 rounded-lg border border-[#2A3648] bg-[#0B1220] px-3 py-1.5 text-xs font-semibold text-slate-200 hover:text-white hover:border-blue-500/40"
          >
            <Plus className="w-3.5 h-3.5" /> Add section
          </button>
        </div>
        {draft.sections.map((s, i) => (
          <div key={i} className="rounded-xl border border-[#2A3648] bg-[#0B1220] p-4 space-y-3">
            <div className="flex items-center gap-2">
              <input
                value={s.heading}
                onChange={e => patchSection(i, { heading: e.target.value })}
                className="flex-1 bg-transparent border-b border-[#2A3648] py-1.5 text-sm font-semibold text-white focus:border-blue-500/50 focus:outline-none"
