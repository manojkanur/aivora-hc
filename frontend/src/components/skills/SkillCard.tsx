...
export function SkillCard({ skill, onClick, showUpgradeCTA = true }: SkillCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'relative rounded-xl border bg-white p-5 transition-shadow',
        isLocked
          ? 'border-border opacity-75 cursor-default'
          : 'border-border shadow-card hover:shadow-card-hover cursor-pointer',
        isRunning && 'border-zinc-400'
      )}
    >
      {isRunning && (
        <div className="absolute inset-0 rounded-xl border-2 border-zinc-400 animate-pulse pointer-events-none" />
      )}

      {isLocked && (
        <div className="absolute inset-0 rounded-xl bg-white/60 flex items-center justify-center z-10">
          <div className="text-center">
            <Lock className="w-5 h-5 text-text-muted mx-auto mb-1" />
            {showUpgradeCTA && (
              <span className="text-xs text-text-muted font-medium">Upgrade to unlock</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="w-10 h-10 rounded-lg bg-surface-tertiary flex items-center justify-center flex-shrink-0">
          {(() => { const Icon = categoryIcons[skill.category] ?? Compass; return <Icon className="w-5 h-5 text-text-secondary" /> })()}
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-text-secondary font-medium">{skill.credit_cost} credits</span>
          </div>
        ...
        {skill.last_run && (
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-text-muted">{formatRelativeTime(skill.last_run)}</span>
          </div>
        )}
      </div>
      ...
      {isRunning && (
        <div className="mt-3 pt-3 border-t border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-text-secondary font-medium">Running...</span>
          </div>
        </div>
      )}
    </motion.div>
