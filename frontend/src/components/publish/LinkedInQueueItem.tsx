...
export function LinkedInQueueItem({ item, onEdit, onSchedule, onPublishNow, onCancel }: LinkedInQueueItemProps) {
  ...
  return (
    ...
    >
      ...
      {item.export_url && (
        <div className="mb-4 p-3 bg-surface-secondary rounded-lg border border-border flex items-center gap-2">
          <div className="w-8 h-8 bg-border rounded flex-shrink-0" />
          <span className="text-xs text-text-muted">Attached export</span>
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
          ...
          <>
            ...
            {onCancel && (
              <Button
                ...
                className="text-text-muted hover:text-red-600 ml-auto"
              >
                Cancel
              </Button>
            )}
        ...
        {item.status === 'published' && (
          <Button variant="ghost" size="sm" leftIcon={<Send className="w-3.5 h-3.5" />} className="text-text-muted">
            View on LinkedIn
          </Button>
        )}
      </div>
