  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <motion.div initial="hidden" animate="visible" variants={fadeUp}>
        ...
        <p className="text-text-secondary mt-1">Manage your plan, credits, and payment</p>
      </motion.div>
      ...
      <div className="bg-white rounded-xl border border-border p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-text-primary">Credits</h3>
          </div>
          <span className="text-sm text-text-secondary">
            <span className="font-bold text-text-primary">{balance}</span> remaining
          </span>
        </div>
      ...
      {/* Plan Options */}
      <div>
        <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-3">Available Plans</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {plans.map(plan => {
            ...
            return (
              <div
                key={plan.id}
                className={`rounded-xl border p-5 ${isCurrent ? 'border-accent bg-accent-light' : 'border-border bg-white'}`}
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-semibold text-text-primary">{plan.name}</p>
                    <p className="text-lg font-bold text-text-primary mt-0.5">{plan.price}</p>
                    <p className="text-xs text-text-muted">{plan.credits ? `${plan.credits} credits/mo` : 'Unlimited'}</p>
                  </div>
      ...
      {transactions.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-3">Transaction History</h3>
          <div className="bg-white rounded-xl border border-border overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-surface-secondary">
                  <th className="text-left text-xs font-semibold text-text-muted px-4 py-3">Description</th>
                  <th className="text-left text-xs font-semibold text-text-muted px-4 py-3 hidden sm:table-cell">Date</th>
                  <th className="text-right text-xs font-semibold text-text-muted px-4 py-3">Credits</th>
                </tr>
              </thead>
              <tbody>
                {transactions.map(tx => (
                  <tr key={tx.id} className="border-b border-border last:border-0">
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <span className="text-sm text-text-muted">{formatDate(tx.created_at)}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className={`text-sm font-semibold ${tx.type === 'topup' ? 'text-zinc-900' : 'text-red-600'}`}>
