            ...
            <div key={step.id} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
                <div className={cn(
                  ...
                  done   ? 'bg-zinc-950 border-zinc-950 text-white' :
                  active ? 'bg-white border-zinc-950 text-zinc-950' :
                ...
                <span className={cn(
                  ...
                  active ? 'text-zinc-950' : done ? 'text-zinc-500' : 'text-zinc-400'
                )}>
                  {step.label}
                </span>
              </div>
              {i < STEPS.length - 1 && (
                <div className={cn(
                  ...
                  completed.includes(step.id) ? 'bg-zinc-950' : 'bg-zinc-200'
                )} />
              )}
            </div>
          )
...
function HowItWorks({ onStart }: { onStart: () => void }) {
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-zinc-900">New Client Brief</h1>
              ...
              <div key={i} className="flex items-start gap-4 px-6 py-4">
                ...
                <div>
                  <p className="text-sm font-semibold text-zinc-900">
                    <span className="text-zinc-400 font-normal mr-2">{i + 1}.</span>
                    {item.title}
                  </p>
      ...
      <div className="flex items-center justify-between p-5 bg-zinc-950 rounded-2xl">
        <div>
          <p className="font-semibold text-white text-sm">Ready to start?</p>
          <p className="text-xs text-zinc-400 mt-0.5">Typical run takes 10–15 minutes end to end</p>
        </div>
        <Button
          onClick={onStart}
...
function StepSkill({
  ...
  return (
    <div className="space-y-4">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                on ? 'bg-zinc-950' : 'bg-white hover:bg-zinc-50'
              )}
            >
              <div className={cn('w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0', on ? 'bg-white/10' : 'bg-zinc-100')}>
                <Icon className={cn('w-4 h-4', on ? 'text-white' : 'text-zinc-500')} />
              </div>
              <div className="flex-1 min-w-0">
                <p className={cn('font-semibold text-sm', on ? 'text-white' : 'text-zinc-900')}>{skill.name}</p>
                <p className={cn('text-xs mt-0.5', on ? 'text-zinc-400' : 'text-zinc-400')}>{skill.desc}</p>
              </div>
              <div className={cn('flex items-center gap-1.5 text-xs font-medium flex-shrink-0', on ? 'text-zinc-400' : 'text-zinc-400')}>
                <Zap className="w-3.5 h-3.5" />{skill.credits} credits
              </div>
              <div className={cn(
                'w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all',
                on ? 'bg-white border-white' : 'border-zinc-300'
              )}>
                {on && <div className="w-2.5 h-2.5 rounded-full bg-zinc-950" />}
              </div>
            </button>
          )
        })}
      ...
      {chosen && (
        <div className="flex items-center gap-3 py-3 px-4 bg-zinc-50 border border-zinc-200 rounded-lg text-sm">
          <CheckCircle2 className="w-4 h-4 text-zinc-600 flex-shrink-0" />
          <span className="text-zinc-700">
            <strong className="text-zinc-900">{chosen.name}</strong> selected — {chosen.credits} credits will be used
          </span>
        </div>
      )}
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3 p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
        <div className="w-8 h-8 rounded-lg bg-zinc-200 flex items-center justify-center flex-shrink-0">
          <skill.icon className="w-4 h-4 text-zinc-600" />
        </div>
        <div>
          <p className="text-sm font-semibold text-zinc-900">{skill.name}</p>
          <p className="text-xs text-zinc-500">Running against: {brief.title}</p>
        </div>
      ...
      {log.length > 0 && (
        <div ref={scrollRef} className="bg-zinc-950 rounded-xl p-4 h-52 overflow-y-auto">
          {log.map((line, i) => (
            <p key={i} className="text-xs text-zinc-400 leading-5">
              <span className="text-zinc-600 mr-2 select-none tabular-nums">{String(i + 1).padStart(2, '0')}</span>
              {line}
            </p>
          ))}
          {phase === 'done' && (
            <p className="text-xs text-zinc-300 font-medium mt-2">Output generated successfully.</p>
          )}
        </div>
      )}
      ...
      {phase === 'idle' && (
        <p className="text-sm text-zinc-500">
          Aivora AI will process your brief and generate a full structured output for <strong className="text-zinc-800">{skill.name}</strong>. This typically takes under a minute.
        </p>
      )}
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3 p-4 bg-zinc-50 border border-zinc-200 rounded-xl">
        ...
        <div>
          <p className="text-sm font-semibold text-zinc-900">{skill.name}</p>
          <p className="text-xs text-zinc-500">Review the generated output below before approving</p>
        </div>
      </div>
      ...
      <div className="border border-zinc-200 rounded-xl overflow-hidden">
        ...
        <div className="p-5 h-72 overflow-y-auto">
          <pre className="text-xs text-zinc-600 leading-relaxed whitespace-pre-wrap font-sans">{output}</pre>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-zinc-200 rounded-xl p-5">
        <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-3">Brief Summary</p>
        <p className="text-base font-bold text-zinc-900 mb-4">{brief.title}</p>
        <div className="grid grid-cols-3 gap-4 text-center pt-4 border-t border-zinc-100">
          <div>
            <div className="text-xl font-bold text-zinc-900">1</div>
            <div className="text-xs text-zinc-500 mt-0.5">Skill run</div>
          </div>
          <div>
            <div className="text-xl font-bold text-zinc-900">{brief.challenges.length}</div>
            <div className="text-xs text-zinc-500 mt-0.5">Challenges addressed</div>
          </div>
          <div>
            <div className="text-xl font-bold text-zinc-900">{skill.credits}</div>
            <div className="text-xs text-zinc-500 mt-0.5">Credits used</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl">
        ...
        <div>
          <p className="text-sm font-semibold text-zinc-900">{skill.name}</p>
          <p className="text-xs text-zinc-500">Ready to download in all formats</p>
        </div>
        <CheckCircle2 className="w-4 h-4 text-zinc-700 ml-auto flex-shrink-0" />
      </div>

      {/* Export formats */}
      <div>
        <p className="text-sm font-semibold text-zinc-900 mb-3">Download Deliverable</p>
        <div className="divide-y divide-zinc-100 border border-zinc-200 rounded-xl overflow-hidden">
            ...
            return (
              <div key={fmt.id} className="flex items-center gap-4 px-5 py-4">
                <div className="flex-1">
                  <p className="text-sm font-semibold text-zinc-900">{fmt.label}</p>
                  <p className="text-xs text-zinc-400 mt-0.5">{fmt.desc}</p>
                </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    done    ? 'bg-zinc-100 text-zinc-500 border-zinc-200 cursor-default' :
                    loading ? 'bg-zinc-900 text-white border-zinc-900 cursor-wait' :
                              'bg-white text-zinc-800 border-zinc-300 hover:border-zinc-600 hover:bg-zinc-50'
                  )}
                >
                  {loading ? (
      ...
      {/* LinkedIn */}
      <div>
        <p className="text-sm font-semibold text-zinc-900 mb-3">Publish Insight</p>
        <div className="border border-zinc-200 rounded-xl px-5 py-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Schedule a thought-leadership post from this client brief</p>
          </div>
          <Button
            size="sm"
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          <button
            ...
            className="text-xs text-zinc-400 hover:text-zinc-700 mb-3 flex items-center gap-1 transition-colors"
          >
            <ChevronLeft className="w-3 h-3" /> How it works
          </button>
          <h1 className="text-2xl font-bold text-zinc-900">{stepMeta?.label}</h1>
          <p className="text-sm text-zinc-500 mt-1">Step {step} of {STEPS.length}</p>
        </div>
        ...
        <div className="mt-4 flex items-center gap-1.5 text-xs text-zinc-400 justify-center">
